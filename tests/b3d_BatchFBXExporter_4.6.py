# ------------------------------------------------------------------------------------------
# EoBatchFBXExportor (B3D to Maya) V4.5a
# ------------------------------------------------------------------------------------------
...
def run_batch():
    ...
    for layer_col in layer_col_sets:
        ...
        # Remove unneeded collections originals.
        bpy.data.collections.remove(layer_col)
    bpy.data.collections.remove(root_col)

    root_col_new.name = rename_recurse(prefix_name, '', root_col_new.name)
    rename_recurse('\.', '_', root_col_new.name)
    export_fbx_recurse(root_col_new.name)
