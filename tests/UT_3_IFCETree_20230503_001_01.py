...
class BlenderOperator:
    ...
    def set_object_parent(self, child: bpy.types.Object, parent: bpy.types.Object, move: bool) -> bpy.types.Object:
    ...
    # def set_object_parent(self, child, parent) -> bpy.types.Object:
    #     child_ob = bpy.data.objects.get(child)
    #     parent_ob = bpy.data.objects.get(parent)
    #     if not parent_ob or not child_ob:
    #         return
    #     coll = self.get_object_collection(parent_ob)
    #     if coll:
    #         hierarchy = self.get_object_hierarchy(child_ob)
    #         for ob in hierarchy:
    #             self.move_object(ob, coll)
    #             # print(f"\"{ob.name}\" has been moved to collection \"{coll.name}\"")
    #     child_ob.parent = parent_ob
    #     # print(f"\"{child_ob.name}\" has been set parent to \"{parent_ob.name}\"")
    ...
    def set_parent_by_select(self) -> bpy.types.Object:
        ...
        for obj in selected_objects:
            if obj != parent_obj:
                self.set_object_parent(parent_obj, obj, True)
    ...
    def add_object(self, name: str, type: str = 'EMPTY', parent: bpy.types.Object = None) -> bpy.types.Object:
        ...
        if parent:
            self.set_object_parent(parent, new_object, True)
...
class IFCParser:
    ...
    def pickup_extras(self):
        # self.ifc_extra_collections = {coll: self.ifc_collections[coll] for coll in self.ifc_collections.keys() if
        #                               coll in self.ifc_extra_list}
        # for ob in self.ifc_objects.keys():
        #     if ob in self.ifc_extra_collections.keys():
        #         self.ifc_extra_objects[ob] = self.ifc_objects[ob]
        for extra in self.ifc_extra_list:
            if extra in self.ifc_collections.keys():
                pop_coll = self.ifc_collections.pop(extra)
                print(f"Collection {pop_coll} has been removed from IFC collections.")
                self.ifc_extra_collections[extra] = {pop_coll.name: pop_coll}
        for coll_name, coll in self.ifc_collections.items():
            print(f"Collection {coll_name} / {coll} has been added to IFC extra collections.")
        for extra_name, extra_coll in self.ifc_extra_collections.items():
            print(f"Collection {extra_name} / {extra_coll} has been added to IFC extra collections.")
...
class IFCBuilder:
    ...
    def assemble(self):
        for obj, ifc_info in self.ifc_info.items():
            ...
            BlenderOperator().set_object_parent(inst, parent, True)
